...
function getWebSession(sname) {
    //sname = "webtracker_flow" + getWebsite();
...
connection.start().then(() => {
    ...
    let user = getCookie(userCookie);
    let web = getCookie(webCookie);
    let flow = getWebSession(flowSession);
    ...
    if(user === "") {
        ...
        console.log("Existing User Connected to New Website " + getWebsite());
        connection.invoke("NewWeb", user, getWebsite(), getUrl());
    }
    else if(flow === ""){
        console.log("Existing User Connected to Existing Website " + getWebsite() + " with a new flow");
        connection.invoke("ExistingWeb", user, web, getWebsite())
    }
    else{
        console.log("Existing user continued flow to the url " + getUrl());
        connection.invoke("ExistingFlow", user, web, getUrl());
    }
...
connection.on("SaveWebSession", (sname, svalue) => {
    sessionStorage.setItem(sname, svalue)
});
