...
{
    const response = await fetch(GEO_LOCATION_PROVIDER);
    var data = await response.json();
    console.log(data)
    connection.invoke("SetLocation", user, JSON.stringify(data))
}
...
connection = new signalR.HubConnectionBuilder()
    ...
    .withUrl("https://localhost:1983/server", {
...
connection.start().then(() => {
    ...
    cname = "webtracker" + getWebsite();
    let user = getCookie(cname);
    if(user === "") {
        console.log("New User Connected to " + getWebsite() + " from " + getBrowserName() + " with " + getDeviceType());
        connection.invoke("AddNewUser", getWebsite(), getDeviceType(), getBrowserName());
    }
    else{
        console.log("Existing User Connected to " + getWebsite() + " from " + getBrowserName() + " with " + getDeviceType());
        connection.invoke("ExistingUser", getWebsite(), user, getDeviceType(), getBrowserName());
    }
    // $.ajax({
    ...
    //         connection.invoke("SetLocation", user, JSON.stringify(location));
    //     }
    //   });
...
connection.on("NewUser", (cname, cvalue, exdays) =>
{
    const d = new Date();
    ...
    let user = getCookie(cname);
    let flow = getWebSession();
    console.log("WebSession",flow);
    if(flow === null) {
        console.log("Website",getWebsite());
        console.log("Url",getUrl());
        connection.invoke("NewFlow", user, getWebsite(), getUrl() + "->")
    }
    else {
        connection.invoke("ExistingFlow", user, getWebsite(), getUrl() + "->")
    }
    saveWebSession();
});
...
window.onload = function() {
    ...
    for(let i = 0; i < forms.length; i++){
        forms[i].onsubmit = (event) => {
            ...
            connection.invoke("ReceiveFormData", JSON.stringify(formProps));
...
// document.forms.addEventListener('submit', (event) => {
//     event.preventDefault();
//     const formData = new FormData(event.target);
//     const formProps = Object.fromEntries(formData);
//     console.log(formProps);
...
document.addEventListener('click', (event) => {
    ...
    if(action != "") {
        connection.invoke("ReceiveAction", action, data);
    }
});
